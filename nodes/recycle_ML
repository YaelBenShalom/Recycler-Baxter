...
class Recycle:
    ...
    def __init__(self):
        ...
        self.can_z = -0.15    # Soda can gripping height for table
        self.bottle_z = -0.15  # Bottle gripping height for table
    ...
    def get_image(self):
        ...
        if len(object_list) > 0:
            ...
            for item in object_list:
                x_pixel = (float(item[1]) + float(item[3])) / 2
                y_pixel = (float(item[2]) + float(item[4])) / 2

                # Calibrate using linearization
                x_meter = x_pixel * 1.92308 + 0.0884615
                y_meter = y_pixel * 2.0 - 0.98

                self.obj_x_list.append(x_meter)
                self.obj_y_list.append(y_meter)
                if int(item[0]) == 0:
    ...
    def get_object_list(self):
        """ Gets the location and label of the detected objects, extracted for a
        txt file produced by the detection pipline
        Args:
          None
          
        Returns:
          object_list (list): a list of objects class and position
        """
        txt_path = '/home/yael/Documents/MSR_Courses/Fall_2020/ME495-Ambedded_Systems_in_Robotics/Final_Project/ws/src/can_sort/detected_objects.txt'
        with open(txt_path, 'r') as f:
            object_list = []
            for line in f:
                stripped_line = line.strip()
                line_list = stripped_line.split()
                object_list.append(line_list)
